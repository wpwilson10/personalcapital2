...
Standalone open-source package. The existing ecosystem (haochi/personalcapital) is abandoned and broken — this client fixes all known issues.
...
- [x] **Phase 3: CLI** — `pc2` argparse command, `--format json`/`--format csv`, relative date args, structured JSON errors, agent-friendly output
- [x] **Phase 4: Richer data** — Full pass-through of all API fields, summary methods, spending endpoint
